...
-- The app needs HockeyData's game ID to request the live game report.
create or replace view public.games_with_my_predictions with (security_invoker = true) as
select g.id, g.external_id, g.season_id, g.phase, g.starts_at, g.home_team_id, home.name home_team_name,
  ...
  g.is_preseason, g.live_elapsed_seconds, g.live_phase
